...
private theorem prop_biprod (P : ObjectProperty A)
    [P.IsClosedUnderBinaryProducts] [P.IsClosedUnderIsomorphisms] {X Y : A}
    (hX : P X) (hY : P Y) : P (X ⊞ Y) := by
  have hprod : P (X ⨯ Y) := P.prop_prod X Y hX hY
  exact P.prop_of_iso (biprod.isoProd X Y).symm hprod
...
private theorem rightPresentation_shortExact {X₁ X₂ X₃ K F : A}
    ...
    (hl : l ≫ g = p) (ht : t ≫ f = i ≫ l) :
    (ShortComplex.mk (biprod.lift (-t) i) (biprod.desc f l) (by
      rw [biprod.lift_desc]
      simp [ht])).ShortExact := by
  haveI := hS.mono_f
  haveI := hS.epi_g
  haveI := hT.mono_f
  haveI := hT.epi_g
  let U : ShortComplex A := ShortComplex.mk (biprod.lift (-t) i) (biprod.desc f l) (by
    rw [biprod.lift_desc]
    simp [ht])
  have hker {W : A} (a : W ⟶ X₁ ⊞ F) (ha : a ≫ biprod.desc f l = 0) :
      (a ≫ biprod.snd) ≫ p = 0 := by
    have hcomp :
        a ≫ biprod.desc f l = (a ≫ biprod.fst) ≫ f + (a ≫ biprod.snd) ≫ l := by
      simp [biprod.desc_eq, comp_add, Category.assoc]
    have h1 : (a ≫ biprod.snd) ≫ l = -((a ≫ biprod.fst) ≫ f) := by
      rw [eq_neg_iff_add_eq_zero]
      simpa [hcomp, add_comm] using ha
    calc
      (a ≫ biprod.snd) ≫ p = ((a ≫ biprod.snd) ≫ l) ≫ g := by
        rw [← hl]
        simp [Category.assoc]
      _ = 0 := by simp [h1, wS]
  ...
  have hepi : Epi U.g := by
    ...
    have hSf_q : f ≫ q = 0 := by
      calc
        f ≫ q = (biprod.inl ≫ biprod.desc f l) ≫ q := by rw [biprod.inl_desc]
        _ = biprod.inl ≫ (biprod.desc f l ≫ q) := by rw [Category.assoc]
        _ = 0 := by rw [hq, comp_zero]
    obtain ⟨d, hd⟩ := hS.exact.desc' q hSf_q
    have hd0 : d = 0 := by
      have hlq : l ≫ q = 0 := by
        calc
          l ≫ q = (biprod.inr ≫ biprod.desc f l) ≫ q := by rw [biprod.inr_desc]
          _ = biprod.inr ≫ (biprod.desc f l ≫ q) := by rw [Category.assoc]
          _ = 0 := by rw [hq, comp_zero]
      have hpd : p ≫ d = 0 := by
        calc
          p ≫ d = l ≫ q := by rw [← hl, Category.assoc, hd]
          _ = 0 := hlq
  ...
  have hexact : U.Exact := by
    ...
    · intro W a ha b hb
      ...
      apply (cancel_mono i).1
      calc
        b ≫ i = (b ≫ biprod.lift (-t) i) ≫ biprod.snd := by
          rw [Category.assoc, biprod.lift_snd]
        _ = a ≫ biprod.snd := by rw [hb]
        _ = m ≫ i := hm.symm
...
private theorem pullback_shortExact {K F X Y : A}
  ...
  have hker {W : A} (a : W ⟶ pullback p t) (ha : a ≫ pullback.snd p t = 0) :
    ...
    calc
      (a ≫ pullback.fst p t) ≫ p = (a ≫ pullback.snd p t) ≫ t := by
...
private theorem horseshoe_middle_shortExact {X₁ X₂ X₃ K₁ F₁ K₃ F₃ : A}
  ...
  have hepi : Epi U.g := by
    ...
    have hpq : (p₁ ≫ f) ≫ q = 0 := by
      calc
        (p₁ ≫ f) ≫ q = (biprod.inl ≫ biprod.desc (p₁ ≫ f) l) ≫ q := by
          rw [biprod.inl_desc]
        _ = biprod.inl ≫ (biprod.desc (p₁ ≫ f) l ≫ q) := by rw [Category.assoc]
        _ = 0 := by rw [hq, comp_zero]
    ...
    have hd0 : d = 0 := by
      have hlq : l ≫ q = 0 := by
        calc
          l ≫ q = (biprod.inr ≫ biprod.desc (p₁ ≫ f) l) ≫ q := by
            rw [biprod.inr_desc]
          _ = biprod.inr ≫ (biprod.desc (p₁ ≫ f) l ≫ q) := by rw [Category.assoc]
          _ = 0 := by rw [hq, comp_zero]
      have hpd : p₃ ≫ d = 0 := by
        calc
          p₃ ≫ d = l ≫ q := by rw [← hl, Category.assoc, hd]
  ...
  have hker₃ {W : A} (a : W ⟶ F₁ ⊞ F₃) (ha : a ≫ m = 0) :
    ...
    calc
      (a ≫ biprod.snd) ≫ p₃ = (a ≫ biprod.snd) ≫ l ≫ g := by rw [hl]
      _ = 0 := by simpa using hqg
  have mk_hpb {W : A} (a : W ⟶ F₁ ⊞ F₃) (ha : a ≫ m = 0)
    ...
    apply (cancel_mono f).1
    calc
      ((a ≫ biprod.fst) ≫ p₁) ≫ f = -((a ≫ biprod.snd) ≫ l) := by
        rw [eq_neg_iff_add_eq_zero]
        simpa [Category.assoc, add_comm] using hsum
      _ = -((k₃ ≫ i₃) ≫ l) := by rw [← hk₃]
      _ = -((k₃ ≫ t) ≫ f) := by rw [ht']
      _ = (k₃ ≫ (-t)) ≫ f := by simp [Category.assoc]
  have hexact : U.Exact := by
    apply ShortComplex.exact_of_f_is_kernel
    refine KernelFork.IsLimit.ofι U.f U.zero ?lift ?fac ?uniq
    · intro W a ha
    ...
    · intro W a ha
      ...
      let k₃ : W ⟶ K₃ := hT₃.exact.lift (a ≫ biprod.snd) (hker₃ a ha)
      have hk₃ : k₃ ≫ i₃ = a ≫ biprod.snd := hT₃.exact.lift_f _ _
      change pullback.lift (a ≫ biprod.fst) k₃ (mk_hpb a ha k₃ hk₃) ≫ j = a
      apply biprod.hom_ext
      · dsimp [j]
        rw [Category.assoc, biprod.lift_fst, pullback.lift_fst]
      · dsimp [j]
        calc
          (pullback.lift (a ≫ biprod.fst) k₃ (mk_hpb a ha k₃ hk₃) ≫
              biprod.lift (pullback.fst p₁ (-t)) (pullback.snd p₁ (-t) ≫ i₃)) ≫
              biprod.snd =
                pullback.lift (a ≫ biprod.fst) k₃ (mk_hpb a ha k₃ hk₃) ≫
                  pullback.snd p₁ (-t) ≫ i₃ := by
            rw [Category.assoc, biprod.lift_snd]
          _ =
              (pullback.lift (a ≫ biprod.fst) k₃ (mk_hpb a ha k₃ hk₃) ≫
                pullback.snd p₁ (-t)) ≫ i₃ := by
            rw [Category.assoc]
          _ = k₃ ≫ i₃ := by rw [pullback.lift_snd]
          _ = a ≫ biprod.snd := hk₃
    · intro W a ha b hb
      dsimp [U] at ha hb
      let k₃ : W ⟶ K₃ := hT₃.exact.lift (a ≫ biprod.snd) (hker₃ a ha)
      have hk₃ : k₃ ≫ i₃ = a ≫ biprod.snd := hT₃.exact.lift_f _ _
      let n : W ⟶ pullback p₁ (-t) := pullback.lift (a ≫ biprod.fst) k₃
        (mk_hpb a ha k₃ hk₃)
      change b = n
      apply pullback.hom_ext
      · calc
          b ≫ pullback.fst p₁ (-t) = (b ≫ j) ≫ biprod.fst := by
            dsimp [j]
            rw [Category.assoc, biprod.lift_fst]
          _ = a ≫ biprod.fst := by rw [hb]
          _ = n ≫ pullback.fst p₁ (-t) := by dsimp [n]; rw [pullback.lift_fst]
      · apply (cancel_mono i₃).1
        calc
          (b ≫ pullback.snd p₁ (-t)) ≫ i₃ = b ≫ pullback.snd p₁ (-t) ≫ i₃ := by
            rw [Category.assoc]
          _ = (b ≫ j) ≫ biprod.snd := by
            dsimp [j]
            rw [Category.assoc, biprod.lift_snd]
          _ = a ≫ biprod.snd := by rw [hb]
          _ = k₃ ≫ i₃ := hk₃.symm
          _ = (n ≫ pullback.snd p₁ (-t)) ≫ i₃ := by dsimp [n]; rw [pullback.lift_snd]
  exact ShortComplex.ShortExact.mk' hexact hmono hepi
...
private theorem pullbackKernelComp_shortExact {X₁ X₂ X₃ K F : A}
  ...
  have hexact : U.Exact := by
    ...
    · intro W a ha
      dsimp [U] at ha
      change pullback.lift a (hS.exact.lift (a ≫ p) (hker a ha)) _ ≫
        pullback.fst p f = a
    ...
    · intro W a ha b hb
      ...
      apply pullback.hom_ext
      · calc
          b ≫ pullback.fst p f = a := hb
          _ = m ≫ pullback.fst p f := by dsimp [m]; rw [pullback.lift_fst]
      · apply (cancel_mono f).1
        calc
          (b ≫ pullback.snd p f) ≫ f = (b ≫ pullback.fst p f) ≫ p := by
            simpa [Category.assoc] using congrArg (fun e => b ≫ e)
              (pullback.condition : pullback.fst p f ≫ p = pullback.snd p f ≫ f).symm
          _ = a ≫ p := by rw [hb]
          _ = (m ≫ pullback.snd p f) ≫ f := by
            dsimp [m]
...
private theorem pullbackLeft_shortExact {X₁ X₂ X₃ F : A}
  ...
  have hmono : Mono U.f := by
    ...
    intro W a ha
    apply (cancel_mono f).1
    have h := congrArg (fun e => e ≫ pullback.snd p g) ha
    simpa [Category.assoc, pullback.lift_snd] using h
  have hepi : Epi U.g := by
    dsimp [U]
    infer_instance
  have hker {W : A} (a : W ⟶ pullback p g) (ha : a ≫ pullback.fst p g = 0) :
    ...
    calc
      (a ≫ pullback.snd p g) ≫ g = (a ≫ pullback.fst p g) ≫ p := by
...
theorem of_shortExact_of_left_of_right {P : ObjectProperty A}
  ...
  let C : A → Prop := fun X₁ =>
    ∀ {X₂ X₃ : A} (f : X₁ ⟶ X₂) (g : X₂ ⟶ X₃) (w : f ≫ g = 0),
      (ShortComplex.mk f g w).ShortExact →
      P.HasFiniteResolution X₃ → P.HasFiniteResolution X₂
  have hmain : C S.X₁ := by
    refine @HasFiniteResolutionOfLength.rec A _ _ P (fun X _ _ => C X) ?_ ?_ _ _ h₁
    · intro X₁ hX₁ X₂ X₃ f g w hS hX₃fin
      haveI := hS.mono_f
      haveI := hS.epi_g
      letI : P.HasFiniteResolution X₃ := hX₃fin
      ...
      | succ T₃ n hT₃ hF₃ hK₃ =>
          ...
          let t : T₃.X₁ ⟶ X₁ := hS.exact.lift (T₃.f ≫ l) (by
          ...
          exact HasFiniteResolution.of_shortExact
            ...
            (prop_biprod P hX₁ hF₃)
    · intro T₁ n hT₁ hF₁ hK₁ ih X₂ X₃ f g w hS hX₃fin
      haveI := hS.mono_f
      haveI := hS.epi_g
      letI : P.HasFiniteResolution X₃ := hX₃fin
      obtain ⟨_, h₃⟩ := hX₃fin.out
      ...
      | succ T₃ n₃ hT₃ hF₃ hK₃ =>
          ...
          haveI : P.HasFiniteResolution (pullback T₁.g (-t)) := hpb
          exact HasFiniteResolution.of_shortExact
            (horseshoe_middle_shortExact hT₁ hS hT₃ l t hl ht)
            (prop_biprod P hF₁ hF₃)
  exact hmain S.f S.g S.zero hS inferInstance
...
theorem of_shortExact_of_middle_of_right {P : ObjectProperty A}
  ...
  have hmain : C S.X₃ := by
    ...
    · intro X₃ hX₃ X₁ X₂ f g w hS hX₂fin
      ...
      haveI : P.HasFiniteResolution (X₁ ⊞ X₃) := hB
    ...
    · intro T₃ n hT₃ hF₃ hK₃ _ X₁ X₂ f g w hS hX₂fin
      ...
      have hB : P.HasFiniteResolution (X₁ ⊞ T₃.X₂) := by
        haveI : P.HasFiniteResolution (pullback T₃.g g) := hN
        exact HasFiniteResolution.of_iso e
