...
theorem Ideal.ufd_iff_height_one_primes_principal [IsDomain R]:
  ...
  · intro hPrincipal
    ...
    · intro hqirr
      ...
      have hspan_ne_top : (span ({q} : Set R)) ≠ ⊤ :=
        fun htop => hqirr.not_isUnit ((span_singleton_eq_top).1 htop)
      rcases exists_le_maximal (span ({q} : Set R)) hspan_ne_top with
        ⟨M, hMmax, hspan_le_M⟩
      ...
      have hq_mem_span_g : q ∈ span {g} := by simpa [hp_span] using hq_mem_p
      rcases (mem_span_singleton.mp hq_mem_span_g) with ⟨a, hqa⟩
      have hg_dvd_q : g ∣ q := ⟨a, hqa⟩
      have hg_ne_zero : g ≠ 0 := by
        intro hg0
        apply hp_ne_bot
        simpa [← hp_span, span_singleton_eq_bot]
      have hg_irr : Irreducible g := Prime.irreducible <|
        (span_singleton_prime hg_ne_zero).1 (by simpa [hp_span] using hpp)
...
variable {R : Type*} [CommRing R] [IsDomain R] [IsNoetherianRing R] (x : R) (hx : Prime x)

theorem Ideal.isPrincipal_of_isPrincipal_localization_away_of_prime
    (hx : Prime x) (p : Ideal R) [p.IsPrime] (hpx : x ∉ p)
  ...
  have hM : Submonoid.powers x ≤ nonZeroDivisors R :=
    powers_le_nonZeroDivisors_of_noZeroDivisors (Prime.ne_zero hx)
  ...
  let S : Set (Ideal R) :=
    {I | I ≤ p ∧ I.IsPrincipal ∧ map
      (algebraMap R (Localization.Away x)) I = map (algebraMap R (Localization.Away x)) p}
  have hSnonempty : S.Nonempty := by
    have hex : ∃ y : R, y ∈ p ∧
        map (algebraMap R (Localization.Away x)) p =
      ...
      have hy_eq : algebraMap R (Localization.Away x) (t * a) =
  ...
  have hJ_ne_bot : map (algebraMap R (Localization.Away x)) p ≠ ⊥ := by
    ...
    have hcbot : comap (algebraMap R (Localization.Away x)) ⊥ = ⊥ := by
      ext a
      show ((algebraMap R (Localization.Away x)) a = 0) ↔ a = 0
      constructor
  ...
  have hy_not_dvd : ¬ x ∣ y := by
    ...
    have hI_le_spanb : I ≤ span {b} := by
      rw [← hIspan, hb]
      exact (span_singleton_le_span_singleton).2 ⟨x, by rw [mul_comm]⟩
...
theorem ufd_of_ufd_localization_away_of_prime (hx : Prime x)
  ...
  intro p hp hheight
  by_cases hxp : x ∈ p
  · let q : Ideal R := Ideal.span ({x} : Set R)
