...
private theorem hasFiniteFreeResolution_quotient_prime_aux [IsNoetherianRing R]
  ...
  have hIA_le_P : Ideal.map (C : R →+* A) I ≤ P := (Ideal.map_le_iff_le_comap).2 <| by simp [hcomap]
  ...
  ·
    ...
    have hquotP : HasFiniteFreeResolution A (A ⧸ P) :=
      hasFiniteFreeResolution_of_linearEquiv (R := A) (Submodule.quotEquivOfEq IA P hPIA.symm) hquot
    ...
    have hN : HasFiniteFreeResolution A N := by
      ...
      have hAnn_I : ∀ r : R, r ∈ I → ∀ x : N, (C r : A) • x = 0 := by
        ...
        have hy0 : (C r : A) • (y : B) = 0 := by
          ...
          have hsmul :
              (C r : A) • (y : B) = (π (C r) : B) * (y : B) := by
            simpa [hAlgebraMap] using
              (Algebra.smul_def (R := A) (A := B) (r := (C r : A)) (x := (y : B)))
          -- now `π (C r) = 0`.
          simp [hsmul, hπCr]
    ...
    let gPP : P →ₗ[A] Pbar :=
      { toFun := fun x => ⟨π x.1, Ideal.mem_map_of_mem π x.2⟩
        ...
        map_smul' := by
          ...
          -- Unfold the scalar actions and reduce to `π.map_mul`.
          -- Left: scalar action on `A` is multiplication.
          rw [smul_eq_mul]
          have hAlgebraMap : (algebraMap A B) = π := rfl
          have hsmulB :
              m • π (↑x : A) = (π m : B) * π (↑x : A) := by
            simpa [hAlgebraMap] using
              (Algebra.smul_def (R := A) (A := B) (r := m) (x := (π (↑x : A) : B)))
          rw [hsmulB]
          exact π.map_mul m (↑x : A) }
