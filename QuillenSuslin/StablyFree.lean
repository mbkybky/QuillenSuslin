...
/-- Let $R$ be a noetherian ring such that every finitely generated projective module over $R$ is
  stably free. Then the same property holds true for $R[x]$.. -/
theorem isStablyFree_polynomial_of_isNoetherianRing [IsNoetherianRing R] (hR : ∀ (P : Type v),
    [AddCommGroup P] → [Module R P] → Module.Finite R P → Projective R P → IsStablyFree R P)
