...
/-!
### A translation lemma (the combinatorial heart of Suslin's argument)

In a bivariate polynomial ring `S[X][Y]`, translating the variable `Y` by a sufficiently large
power of `X` turns a polynomial whose leading coefficient (as a polynomial in `Y`) is monic in `X`
into a polynomial which is monic in `X` (after swapping the two variables).
-/
namespace SuslinMonicTranslation
...
theorem exists_K_monic_swap_algEquivAevalXAddC (p : S[X][Y]) (hp : p.leadingCoeff.Monic) :
    ∃ K : ℕ,
      (Polynomial.Bivariate.swap (R := S)
          ((Polynomial.algEquivAevalXAddC ((X : S[X]) ^ K)) p)).Monic := by
  classical
  ...
  let swapS : S[X][Y] ≃ₐ[S] S[X][Y] := Polynomial.Bivariate.swap (R := S)
  ...
  have hswap :
      swapS (τ p) = ∑ i ∈ Finset.range (N + 1), term i := by
  ...
  have hswap' : swapS (τ p) = rest + main := by
    ...
    have hsum :
        (∑ i ∈ Finset.range (N + 1), term i) = rest + main := by
  ...
  have hswapLC_monic : (swapS (C p.leadingCoeff : S[X][Y])).Monic := by
    ...
    have hswapC :
        swapS (C p.leadingCoeff : S[X][Y]) =
          Polynomial.map (C : S →+* S[X]) p.leadingCoeff := by
      simpa [swapS] using (SuslinMonicAux.bivariate_swap_C (A := S) (p := p.leadingCoeff))
  ...
  have hdeg_term :
    ...
    have hdeg_swapCi :
        (swapS (C (p.coeff i) : S[X][Y])).degree ≤ (M : WithBot ℕ) := by
      -- Convert via natDegree.
      have hnat :
          (swapS (C (p.coeff i) : S[X][Y])).natDegree = (p.coeff i).natDegree := by
        have hswapC :
            swapS (C (p.coeff i) : S[X][Y]) =
              Polynomial.map (C : S →+* S[X]) (p.coeff i) := by
          simpa [swapS] using (SuslinMonicAux.bivariate_swap_C (A := S) (p := p.coeff i))
        simpa [hswapC] using
    ...
    have hdeg_mul :
        (term i).degree ≤ (M : WithBot ℕ) + (((N - 1) * K : ℕ) : WithBot ℕ) := by
  ...
  have hdeg_rest :
      rest.degree ≤ bound := by
    -- Use `degree_sum_le` and the uniform bound for summands.
    have hsup :
        (Finset.range N).sup (fun i => (term i).degree) ≤ bound := by
  ...
  have hdeg_lt : rest.degree < main.degree := by
    ...
    -- Compute `main.degree`.
    have hnat_swapLC :
        (swapS (C p.leadingCoeff : S[X][Y])).natDegree = p.leadingCoeff.natDegree := by
    ...
    have hnat_main : main.natDegree = p.leadingCoeff.natDegree + N * K := by
      ...
      simpa [main, term, hcoeffN, hnat_swapLC, hnat_baseN, add_comm, add_left_comm, add_assoc] using this
    have hmain_deg :
        main.degree = ((p.leadingCoeff.natDegree + N * K : ℕ) : WithBot ℕ) := by
...
theorem suslin_monic_polynomial_theorem [IsNoetherianRing R] (hr : ringKrullDim R < ⊤) (n : ℕ)
  ...
  | zero =>
    ...
    have hLC_top : (J.leadingCoeff : Ideal R) = ⊤ := by
      ...
      have hbound :
          (J.leadingCoeff.height : WithBot ℕ∞) ≤ ringKrullDim R := by
    ...
    rcases (Ideal.mem_map_iff_of_surjective (f := e0.toRingHom) e0.toRingEquiv.surjective).1 hgJ with
      ⟨f, hfI, rfl⟩
    ...
    have hf_map :
        e0 f = Polynomial.map (eEmpty : MvPolynomial (Fin 0) R →+* R) ((MvPolynomial.finSuccEquiv R 0) f) := by
      simp [e0, Polynomial.mapAlgEquiv, Polynomial.mapAlgHom]
    have hLC_map :
        (Polynomial.map (eEmpty : MvPolynomial (Fin 0) R →+* R) ((MvPolynomial.finSuccEquiv R 0) f)).leadingCoeff =
          (eEmpty : MvPolynomial (Fin 0) R →+* R) (((MvPolynomial.finSuccEquiv R 0) f).leadingCoeff) := by
      simpa using
    ...
    have hLC_image :
        (eEmpty : MvPolynomial (Fin 0) R →+* R) (((MvPolynomial.finSuccEquiv R 0) f).leadingCoeff) = 1 := by
      have : (Polynomial.map (eEmpty : MvPolynomial (Fin 0) R →+* R) ((MvPolynomial.finSuccEquiv R 0) f)).leadingCoeff = 1 := by
    ...
    have hLC :
      ...
      have : (eEmpty : MvPolynomial (Fin 0) R →+* R) (((MvPolynomial.finSuccEquiv R 0) f).leadingCoeff) =
  ...
  | succ n ih =>
    ...
    let eLast : A ≃ₐ[R] Polynomial B := MvPolynomial.finSuccEquivLast (R := R) (n + 1)
    ...
    rcases
        (Ideal.mem_map_iff_of_surjective (f := eLast.toRingHom) eLast.toRingEquiv.surjective).1 hqJ with
      ⟨f0, hf0I, hf0eq⟩
    ...
    -- Identify `B` as a polynomial ring in `x₀` over `S`, then view `A` as a bivariate polynomial ring.
    ...
    -- Set `p` to be the image of `f0` in the bivariate polynomial ring after the coefficient change.
    ...
    have hp_lc : p.leadingCoeff = eX (eB g) := by
      ...
      have h₁ : eLast (eExt f0) = Polynomial.map (eB : B →+* B) (eLast f0) := by
        have hf :
            eExt f0 = eLast.symm (Polynomial.map (eB : B →+* B) (eLast f0)) := by
      ...
      have hLC_q1 : (eLast (eExt f0)).leadingCoeff = eB g := by
        have hLC_mapB :
            (Polynomial.map (eB : B →+* B) (eLast f0)).leadingCoeff = eB (eLast f0).leadingCoeff := by
          simpa using
            (Polynomial.leadingCoeff_map_of_injective
              (f := (eB : B →+* B))
      ...
      have hLC_mapX :
          (Polynomial.map (eX : B →+* Polynomial S) (eLast (eExt f0))).leadingCoeff =
    ...
    rcases SuslinMonicTranslation.exists_K_monic_swap_algEquivAevalXAddC (S := S) p hp_lc_monic with
      ⟨K, hmonic_swap⟩
    ...
    have hEq :
        (Polynomial.mapAlgEquiv (MvPolynomial.finSuccEquivLast (R := R) n)
          ...
          Polynomial.Bivariate.swap (R := S) (H (eTotal f0)) := by
      -- `finSuccEquiv_map_finSuccEquivLast_apply` rewrites the LHS as a swapped bivariate form.
      simpa [H, eLast, eX] using
        (SuslinMonicAux.finSuccEquiv_map_finSuccEquivLast_apply (R := R) n (eTotal f0))
    have hLHS_monic :
        (Polynomial.mapAlgEquiv (MvPolynomial.finSuccEquivLast (R := R) n)
    ...
    have hLC_q0 : q0.leadingCoeff = 1 := by
      ...
      let eCoeff : B →+* Polynomial S := (MvPolynomial.finSuccEquivLast (R := R) n : B →+* Polynomial S)
      have hLC_map : (Polynomial.map eCoeff q0).leadingCoeff = eCoeff q0.leadingCoeff := by
        simpa [eCoeff] using
          (Polynomial.leadingCoeff_map_of_injective
            (f := eCoeff) (hf := (MvPolynomial.finSuccEquivLast (R := R) n).toRingEquiv.injective) (p := q0))
      ...
      have :
        ...
        simpa using hLC_image
      exact (MvPolynomial.finSuccEquivLast (R := R) n).toRingEquiv.injective this
